...
@section('title', 'Publier un hôtel')
...
@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 max-w-4xl">
        <nav class="flex items-center gap-2 text-sm text-muted-foreground dark:text-gray-400 mb-6">
            ...
            <i data-lucide="chevron-right" class="w-3 h-3"></i>
            <span class="text-foreground dark:text-gray-300">Publier un hôtel</span>
        </nav>

        <h1 class="text-2xl md:text-3xl font-bold text-foreground dark:text-white mb-6">Publier un hôtel</h1>
        ...
        <form action="{{ route('hotel.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required minlength="8"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4"
                            class="w-full px-4 py-2.5 border border-border dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-ring dark:focus:ring-primary/30 bg-background dark:bg-gray-900 text-foreground dark:text-white @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="contact" value="{{ old('contact') }}" placeholder="+221 77 123 45 67"
                    ...
                    <div>
                        ...
                        <select name="star_rating" required
                            ...
                            @for($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" {{ old('star_rating', 3) == $i ? 'selected' : '' }}>{{ $i }} étoile(s)</option>
                            @endfor
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="rooms" value="{{ old('rooms', 1) }}" required min="1"
                            class="w-full px-4 py-2.5 border border-border dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-ring dark:focus:ring-primary/30 bg-background dark:bg-gray-900 text-foreground dark:text-white">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-foreground dark:text-gray-300 mb-1">Salles de bain *</label>
                        <input type="number" name="bathrooms" value="{{ old('bathrooms', 1) }}" required min="0"
                            class="w-full px-4 py-2.5 border border-border dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-ring dark:focus:ring-primary/30 bg-background dark:bg-gray-900 text-foreground dark:text-white">
                    </div>
                ...
                <div x-show="showHourlyPrices" x-collapse class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    @for($hour = 1; $hour <= 24; $hour++)
                        <div>
                            ...
                            <input type="number" name="hourly_prices[{{ $hour }}]" value="{{ old('hourly_prices.' . $hour) }}" min="0" placeholder="Prix"
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="address" value="{{ old('address') }}" required minlength="8"
                    ...
                    <div>
                        ...
                        <select name="arrondissement_id"
                            ...
                            @foreach($arrondissements as $arrondissement)
                                <option value="{{ $arrondissement->id }}" {{ old('arrondissement_id') == $arrondissement->id ? 'selected' : '' }}>{{ $arrondissement->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <!-- Images -->
            <div>
                <h2 class="text-lg font-semibold text-foreground dark:text-white mb-4">Photos</h2>
            ...
            <div>
                <label class="flex items-start gap-3 cursor-pointer">
                    <input type="checkbox" name="conditions" value="1" required
                        class="mt-1 h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-primary focus:ring-primary">
                    <span class="text-sm text-foreground dark:text-gray-300">
                        J'accepte les <a href="{{ route('conditions') }}" class="text-primary hover:underline">conditions d'utilisation</a> et je certifie que les informations fournies sont exactes.
                    </span>
                </label>
                @error('conditions') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
            </div>
