...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-gray-800 dark:text-white">Modifier le bureau</h1>
    ...
    <x-container-dashed>
        <form action="{{ route($adminRoutePrefix . '.update', $bureau) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')

            <input type="hidden" name="is_active" value="{{ $bureau->is_active ? 1 : 0 }}">
            <input type="hidden" name="is_verify" value="{{ $bureau->is_verify ? 1 : 0 }}">
            ...
            <x-form.textarea name="description" label="Description" rows="4">{{ old('description', $bureau->description) }}</x-form.textarea>
            ...
            <div class="flex gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <a href="{{ route($adminRoutePrefix . '.show', $bureau) }}" class="inline-flex items-center gap-2 px-4 py-2 text-gray-600 dark:text-gray-400 hover:text-primary dark:hover:text-primary-400">
