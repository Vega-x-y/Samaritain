...
@section('title', 'Les boutiques')

@section('content')
    @php
        $propertyType = 'boutique';
        $adminRoutePrefix = 'admin.boutique';
        $propertyLabel = 'boutique';
    @endphp
    @if (!$boutiques->isEmpty())
        <div class="flex justify-between">
            <h1 class="text-gray-800 dark:text-white">Liste des boutiques</h1>
                            ...
                            @foreach ($boutiques as $boutique)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                    ...
                                    <td class="px-4 py-3 font-medium hover:text-blue-600 dark:hover:text-blue-400">
                                        <a href="{{ route($adminRoutePrefix . '.show', $boutique) }}" class="text-gray-800 dark:text-white">{{ $boutique->name }}</a>
                                    </td>
                                    <td class="px-4 py-3 text-gray-800 dark:text-white">{{ $boutique->location ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $boutique->phone ?? '-' }}</td>
                                    <td class="px-4 py-3">{{ $boutique->email ?? '-' }}</td>
                                    <td class="px-4 py-3">
                                        @if ($boutique->active)
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex justify-center gap-2">
                                            <a href="{{ route($adminRoutePrefix . '.show', $boutique) }}" class="text-blue-600 dark:text-blue-400 hover:underline">
                                                <i data-lucide="eye" class="w-4 h-4"></i>
                                            </a>
                                            <a href="{{ route($adminRoutePrefix . '.edit', $boutique) }}" class="text-amber-600 dark:text-amber-400 hover:underline">
                                                <i data-lucide="edit" class="w-4 h-4"></i>
                                            </a>
                                            <button type="button" @click="openModal('{{ route($adminRoutePrefix . '.destroy', $boutique) }}')" class="text-red-600 dark:text-red-400 hover:underline">
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="text-gray-600 dark:text-gray-400 mb-4">Commencez en ajoutant votre première boutique</p>
            <x-btn href="{{ route($adminRoutePrefix . '.create') }}" class="dark:bg-primary-600 dark:text-white dark:hover:bg-primary-700">
                <x-slot:prefix>
                    <i data-lucide="plus"></i>
                </x-slot:prefix>
