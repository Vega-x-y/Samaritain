...
@section('content')
    @if (!$amenities->isEmpty())
        <div class="flex justify-between items-center">
            <h1 class="text-gray-800 dark:text-white">Équipements</h1>
            <x-btn href="{{ route('admin.configuration.amenity.create') }}" class="dark:bg-primary-600 dark:text-white dark:hover:bg-primary-700">
                <x-slot:prefix>
        ...
        <!-- Recherche -->
        <form method="GET" action="{{ route('admin.configuration.amenity.index') }}" class="flex items-center gap-2">
            ...
            @if (request('search'))
                <a href="{{ route('admin.configuration.amenity.index') }}" class="text-xs text-gray-500 dark:text-gray-400 hover:text-primary">
                    Réinitialiser
                </a>
            @endif
        </form>
        ...
        <x-container-dashed>
            <div x-data="amenityActions()" @keydown.escape="closeDeleteModal()">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @foreach ($amenities as $amenity)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                    ...
                                    <td class="px-4 py-3 text-center text-gray-800 dark:text-white">{{ $amenity->sort_order }}</td>
                                    <td class="px-4 py-3 text-center">
                                        <span class="px-2 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-full">
                                            {{ $amenity->properties_count }} bien(s)
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('admin.configuration.amenity.edit', $amenity) }}" class="block text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300" title="Modifier">
                                                <i data-lucide="edit" class="w-4 h-4"></i>
                                            </a>
                                            <form action="{{ route('admin.configuration.amenity.toggle-active', $amenity) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="block text-yellow-500 dark:text-yellow-400 hover:text-yellow-600 dark:hover:text-yellow-300" title="{{ $amenity->is_active ? 'Désactiver' : 'Activer' }}">
                                                    <i data-lucide="{{ $amenity->is_active ? 'toggle-right' : 'toggle-left' }}" class="w-4 h-4"></i>
                                                </button>
                                            </form>
                                            <button x-on:click="openDeleteModal('{{ route('admin.configuration.amenity.destroy', $amenity) }}', '{{ $amenity->name }}', {{ $amenity->properties_count }})" class="block text-destructive dark:text-red-400 hover:text-red-600 dark:hover:text-red-300" title="Supprimer">
                ...
                <div x-cloak x-show="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70" x-on:click.self="closeDeleteModal()">
                    <div class="relative w-full max-w-md rounded-lg bg-background dark:bg-gray-800 p-6 shadow-lg" x-on:click.stop>
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Supprimer l'équipement</h3>
                                <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                    Êtes-vous sûr de vouloir supprimer <strong x-text="amenityName" class="text-gray-800 dark:text-white"></strong> ? Cette action est irréversible.
                                </p>
                                <p x-show="usageCount > 0" class="mt-2 text-sm text-red-600 dark:text-red-400">
                                    ⚠️ Cet équipement est utilisé par <span x-text="usageCount"></span> bien(s). Vous pouvez le désactiver au lieu de le supprimer.
                                </p>
                            </div>
                        </div>
    ...
    @else
        ...
        <x-empty title="Aucun équipement trouvé" description="Créez un premier équipement pour commencer" class="dark:text-gray-400">
            <x-slot:icon>
                <i data-lucide="sparkles" class="text-gray-400 dark:text-gray-500"></i>
            </x-slot:icon>
        </x-empty>
    @endif

    <script>
        function amenityActions() {
            return {
