...
@section('title', 'Ajouter une catégorie de parcelle')
...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-700 dark:text-gray-200">Ajouter une catégorie de parcelle</h1>
            <p class="text-gray-500 dark:text-gray-400 text-xs mt-1">Créez une nouvelle catégorie de parcelle.</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div class="p-6 md:p-8">
                <form action="{{ route('admin.configuration.parcelle-category.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <x-form.input name="name" label="Nom de la catégorie *" placeholder="Ex: Terrain nu" required />
                        <x-form.input name="slug" label="Slug (optionnel)" placeholder="Ex: terrain-nu" />
                    </div>
