...
@section('title', 'Catégories de parcelles')

@section('content')
    @if (!$categories->isEmpty())
        <div class="flex justify-between items-center">
            <h1 class="text-gray-800 dark:text-white">Catégories de parcelles</h1>
            <x-btn href="{{ route('admin.configuration.parcelle-category.create') }}" class="dark:bg-primary-600 dark:text-white dark:hover:bg-primary-700">
                <x-slot:prefix>
        ...
        <!-- Recherche -->
        <form method="GET" action="{{ route('admin.configuration.parcelle-category.index') }}" class="flex items-center gap-2">
            ...
            @if (request('search'))
                <a href="{{ route('admin.configuration.parcelle-category.index') }}" class="text-xs text-gray-500 dark:text-gray-400 hover:text-primary">
                    Réinitialiser
                </a>
            @endif
        </form>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span class="px-2 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-full">
                                            {{ $category->parcelles_count }} parcelle(s)
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('admin.configuration.parcelle-category.edit', $category) }}" class="block text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300" title="Modifier">
                                                <i data-lucide="edit" class="w-4 h-4"></i>
                                            </a>
                                            <form action="{{ route('admin.configuration.parcelle-category.toggle-active', $category) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <button x-on:click="openDeleteModal('{{ route('admin.configuration.parcelle-category.destroy', $category) }}', '{{ $category->name }}', {{ $category->parcelles_count }})" class="block text-destructive dark:text-red-400 hover:text-red-600 dark:hover:text-red-300" title="Supprimer">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <p x-show="usageCount > 0" class="mt-2 text-sm text-red-600 dark:text-red-400">
                                    ⚠️ Cette catégorie est utilisée par <span x-text="usageCount"></span> parcelle(s). Vous pouvez la désactiver au lieu de la supprimer.
                                </p>
                            </div>
                        </div>
