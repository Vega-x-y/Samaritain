...
@section('title', 'Modifier la catégorie d\'artisan')
...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-700 dark:text-gray-200">Modifier la catégorie d'artisan</h1>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div class="p-6 md:p-8">
                <form action="{{ route('admin.configuration.artisan-category.update', $category) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <x-form.input name="name" label="Nom de la catégorie *" placeholder="Ex: Plombier" value="{{ old('name', $category->name) }}" required />
                        <x-form.input name="slug" label="Slug (optionnel)" placeholder="Ex: plombier" value="{{ old('slug', $category->slug) }}" />
                    </div>
