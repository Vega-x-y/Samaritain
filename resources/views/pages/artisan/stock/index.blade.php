...
<div class="container mx-auto px-4 py-8" x-data="stockApp()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Mon <span class="text-orange-500">stock</span></h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Gérez vos articles et matériaux</p>
        </div>
        <a href="{{ route('artisan.stock.create') }}" class="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-full font-semibold text-sm transition shadow-md hover:shadow-lg">
            + Nouvel article
        </a>
    </div>
    ...
    <div class="flex flex-wrap gap-2 items-center mb-6">
        <a href="{{ route('artisan.stock.index') }}"
            class="px-4 py-1.5 rounded-full text-sm font-medium border transition
                {{ !request('categorie') && !request('search') ? 'bg-gray-900 text-white border-gray-900 dark:bg-white dark:text-gray-900' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-orange-500' }}">
            📊 Tous
        </a>
        @foreach ($categories as $categorie)
            <a href="{{ route('artisan.stock.index', ['categorie' => $categorie]) }}"
                class="px-4 py-1.5 rounded-full text-sm font-medium border transition
                    {{ request('categorie') === $categorie ? 'bg-gray-900 text-white border-gray-900 dark:bg-white dark:text-gray-900' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-orange-500' }}">
                {{ $categorie }}
            </a>
        @endforeach
        ...
        @forelse ($articles as $article)
            <div class="bg-sidebar dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-accent dark:border-gray-700 transition hover:-translate-y-1 hover:shadow-md hover:border-orange-300 dark:hover:border-orange-700">
                ...
                <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('artisan.stock.show', $article) }}" class="flex-1 bg-orange-500 hover:bg-orange-600 text-white text-center py-2 rounded-full text-sm font-medium transition">
                        Voir détails
                    </a>
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">📦 Nouvel article</h2>
            ...
            <form method="POST" action="{{ route('artisan.stock.store') }}">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="categorie" value="{{ old('categorie') }}" placeholder="Ex: Plomberie, Électricité..."
                                class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg focus:border-orange-500 focus:ring-2 focus:ring-orange-100 dark:focus:ring-orange-900 outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                ...
                <div class="mt-6">
                    <button type="submit" class="w-full bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-xl font-semibold transition">Créer l'article</button>
                </div>
            </form>
        </div>
    </div>
