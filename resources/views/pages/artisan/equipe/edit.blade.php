...
@section('title', 'Modifier membre - Artisan')
...
<div class="container mx-auto px-4 py-8">
    <!-- En-tête -->
    <div class="mb-8">
        <h1 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white"><i data-lucide="pencil" class="w-4 h-4 inline-block align-middle mr-1"></i> Modifier le <span class="text-primary">membre</span></h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Modifiez les informations du membre</p>
    </div>

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('artisan.equipe.update', $membre) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="role" value="{{ old('role', $membre->role) }}" required placeholder="Ex: Maçon, �?lectricien, Peintre..."
                            class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/10 dark:focus:ring-primary/20 outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="telephone" value="{{ old('telephone', $membre->telephone) }}" required
                                class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg focus:border-primary focus:ring-2 focus:ring-primary/10 dark:focus:ring-primary/20 outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    ...
                    <div>
                        ...
                        <x-form.select name="statut" label="Statut *" :options="$statuts->mapWithKeys(fn ($statut) => [$statut->value => $statut->label()])" :value="$membre->statut->value" required />
                    </div>
                </div>
            </div>
