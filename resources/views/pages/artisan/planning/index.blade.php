...
<div class="container mx-auto px-4 py-8" x-data="planningApp()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Mon <span class="text-orange-500">planning</span></h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Gérez vos interventions et rendez-vous</p>
        </div>
        <a href="{{ route('artisan.planning.create') }}" class="bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-full font-semibold text-sm transition shadow-md hover:shadow-lg">
            + Nouvel événement
        </a>
    </div>
    ...
    <div class="flex flex-wrap gap-2 items-center mb-6">
        <a href="{{ route('artisan.planning.index') }}"
            class="px-4 py-1.5 rounded-full text-sm font-medium border transition
                {{ !request('type') && !request('chantier_id') ? 'bg-gray-900 text-white border-gray-900 dark:bg-white dark:text-gray-900' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-orange-500' }}">
            📊 Tous
        </a>
    ...
    <div class="bg-sidebar dark:bg-gray-800 rounded-xl border border-accent dark:border-gray-700 shadow-sm overflow-hidden">
        ...
        <div class="grid grid-cols-7 auto-rows-fr">
            ...
            @foreach ($days as $day)
                ...
                @endphp
                <div class="min-h-[100px] border-r border-b border-gray-200 dark:border-gray-700 last:border-r-0 p-2 transition hover:bg-gray-50 dark:hover:bg-gray-700/50 {{ !$isCurrentMonth ? 'bg-gray-50/50 dark:bg-gray-900/30' : 'bg-white dark:bg-gray-800' }} {{ $dayEvents->count() > 0 ? 'ring-1 ring-inset ring-orange-200 dark:ring-orange-900/50' : '' }}">
                    <div class="flex items-center justify-center w-7 h-7 rounded-full text-sm font-medium mb-2 {{ $isToday ? 'bg-orange-500 text-white' : ($isCurrentMonth ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-600') }}">
                        {{ $day->day }}
                    </div>
                    ...
                    @if ($dayEvents->count() > 0)
                        <div class="flex justify-center -mt-1.5 mb-1">
                            <span class="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" title="{{ $dayEvents->count() }} événement(s) ce jour"></span>
                        </div>
                    @endif
                        ...
                        @foreach ($dayEvents as $event)
                            <div class="group relative">
                                ...
                                <div class="hidden group-hover:flex absolute z-10 left-0 top-full mt-1 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-2 gap-1">
                                    <a href="{{ route('artisan.planning.edit', $event) }}" class="px-2 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600 transition">✏️</a>
                                    <form method="POST" action="{{ route('artisan.planning.destroy', $event) }}" class="inline" onsubmit="return confirm('Supprimer cet événement ?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-2 py-1 bg-red-500 text-white rounded text-xs hover:bg-red-600 transition">🗑️</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
    ...
    <div class="mt-8">
        ...
        @if ($evenements->isEmpty())
            <div class="bg-sidebar dark:bg-gray-800 rounded-xl border border-accent dark:border-gray-700 p-8 text-center">
                ...
                <a href="{{ route('artisan.planning.create') }}" class="inline-block mt-4 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded-full font-semibold text-sm transition shadow-md hover:shadow-lg">
                    + Créer un événement
                </a>
            </div>
        @else
            <div class="bg-sidebar dark:bg-gray-800 rounded-xl border border-accent dark:border-gray-700 overflow-hidden">
    ...
    <div x-show="modalOpen" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-bold text-gray-900 dark:text-white">📅 Nouvel événement</h2>
            ...
            <form method="POST" action="{{ route('artisan.planning.store') }}">
                ...
                <div class="mt-6">
                    <button type="submit" class="w-full bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-xl font-semibold transition">Créer l'événement</button>
                </div>
            </form>
        </div>
    </div>
