...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700">
            ...
            <div class="flex gap-2 mt-3 flex-wrap">
                <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ in_array($chantier->statut->value, ['devis','en_cours','attente','arret','termine']) ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400' }}">Devis</span>
                <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ in_array($chantier->statut->value, ['en_cours','attente','arret','termine']) ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400' }}">Chantier</span>
                <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ in_array($chantier->statut->value, ['attente','termine']) ? 'bg-primary text-white' : ($chantier->statut->value === 'en_cours' ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-400') }}">Validation</span>
                <span class="px-2 py-0.5 rounded-full text-xs font-semibold {{ $chantier->statut->value === 'termine' ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400' }}">Livré</span>
            </div>
        </div>
                ...
                <div class="space-y-2">
                    @foreach ($checklist as $item)
                        ...
                        <div class="flex items-center gap-2 py-1 border-b border-gray-50 dark:border-gray-700/50 {{ $done ? 'line-through text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-300' }}">
                            <input type="checkbox" {{ $done ? 'checked' : '' }} disabled class="w-4 h-4 accent-primary">
    ...
    <!-- Actions de statut -->
    <div class="mt-8 bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-100 dark:border-gray-700">
