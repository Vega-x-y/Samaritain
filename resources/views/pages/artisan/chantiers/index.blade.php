...
<div class="container mx-auto px-4 py-8" x-data="chantierApp()">
    ...
    <div class="mb-6 space-y-3">
        <x-artisan.filter-pills name="statut" :options="\App\Enums\ChantierStatus::cases()" allLabel="Tous les statuts" />
        <x-artisan.filter-pills name="type" :options="$types" allLabel="Tous les types" :icons="$typeIcons ?? []" />
        <x-artisan.active-filters
            :filters="[
                ['name' => 'statut', 'label' => 'Statut', 'labels' => collect(\App\Enums\ChantierStatus::cases())->mapWithKeys(fn ($s) => [$s->value => $s->label()])->all()],
                ['name' => 'type', 'label' => 'Type', 'labels' => $types],
            ]"
            :total="$chantiers->total()" totalLabel="chantier(s)" />
    </div>
