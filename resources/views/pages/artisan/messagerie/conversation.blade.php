...
@section('content')
<div class="fixed inset-0 z-[60] flex h-[100dvh] w-full flex-col md:static md:mx-auto md:h-full md:max-h-[calc(100dvh-7rem)] md:max-w-5xl" x-data="conversationApp({{ $conversation->id }})">
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden bg-white dark:bg-gray-800 md:rounded-3xl md:border md:border-gray-200 md:shadow-sm md:shadow-gray-200/40 md:ring-1 md:ring-gray-100 md:dark:border-gray-700 md:dark:ring-gray-700/70">
        <div class="flex shrink-0 items-center justify-between gap-3 border-b border-gray-200 bg-gradient-to-r from-primary/5 via-white to-white px-3 py-3 dark:border-gray-700 dark:from-primary/10 dark:via-gray-800 dark:to-gray-800 sm:px-6 sm:py-4">
            <div class="flex min-w-0 items-center gap-3 sm:gap-4">
                <a href="{{ route('artisan.messagerie.index') }}" aria-label="Retour à la messagerie" class="shrink-0 rounded-full p-2 text-gray-600 transition hover:bg-primary/10 hover:text-primary dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-primary md:hidden">
        ...
        <div x-data="{ showPaymentModal: false, totalAmount: '', downPayment: '' }">
            ...
            <form method="POST" action="{{ route('artisan.messagerie.message', $conversation) }}" class="flex shrink-0 flex-col gap-2 border-t border-gray-200 bg-white px-3 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] dark:border-gray-700 dark:bg-gray-800 sm:px-6 sm:py-3" enctype="multipart/form-data" x-data="{ fileName: '' }">
                @csrf
