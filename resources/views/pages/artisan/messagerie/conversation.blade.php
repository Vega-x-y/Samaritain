                        ...
                        <div class="max-w-[70%] {{ $message->expediteur_type === 'artisan' ? 'bg-orange-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white' }} rounded-lg px-4 py-2">
                            ...
                            @if ($message->fichier_path)
                                <div class="mt-2 {{ $message->contenu ? 'pt-2 border-t border-white/20' : '' }}">
                                    <a href="{{ Storage::disk('r2')->url($message->fichier_path) }}" target="_blank" class="flex items-center gap-2 text-sm {{ $message->expediteur_type === 'artisan' ? 'text-orange-100 hover:text-white' : 'text-orange-600 dark:text-orange-400 hover:text-orange-700' }}">
                            ...
                            <div class="text-xs mt-1 flex items-center gap-1 {{ $message->expediteur_type === 'artisan' ? 'text-orange-100' : 'text-gray-500 dark:text-gray-400' }}">
                                {{ $message->created_at->format('d/m/Y H:i') }}
                                @if ($message->lu)
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
                    ...
                    <form method="POST" action="{{ route('artisan.payment-link.store', $conversation) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="number" name="total_amount" x-model="totalAmount" min="1" required
                                       class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-orange-500 focus:ring-orange-500"
                        ...
                        <div class="flex gap-3 mt-6">
                            ...
                            <button type="submit"
                                    class="flex-1 py-2 px-4 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition text-sm font-medium">
                                Envoyer le lien
                            </button>
                        </div>
                    </form>
            ...
            <form method="POST" action="{{ route('artisan.messagerie.message', $conversation) }}" class="flex flex-col gap-2" enctype="multipart/form-data" x-data="{ fileName: '' }">
                ...
                <div class="flex gap-2">
                    ...
                    <button type="button" @click="showPaymentModal = true"
                            class="bg-orange-100 dark:bg-orange-900/30 hover:bg-orange-200 dark:hover:bg-orange-800/40 text-orange-600 dark:text-orange-400 px-4 py-2.5 rounded-lg transition" title="Demander un acompte">
