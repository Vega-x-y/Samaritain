...
@section('title', 'Nouveau groupe - Messagerie')

@section('breadcrumbs')
    <nav class="flex items-center gap-2 text-sm text-muted-foreground">
        <a href="{{ route('artisan.messagerie.index') }}" class="hover:text-orange-500 transition-colors">
...
<div class="container mx-auto px-4 py-8">
    <!-- En-tête -->
    <div class="mb-8">
        <h1 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">👥 Nouveau <span class="text-orange-500">groupe</span></h1>
        <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Créez un nouveau groupe de discussion</p>
    </div>

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('artisan.messagerie.groupes.store') }}" class="space-y-6">
            @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            class="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg focus:border-orange-500 focus:ring-2 focus:ring-orange-100 dark:focus:ring-orange-900 outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('description') }}</textarea>
